...
class StockMoveLine(models.Model):
    ...
    user_can_edit_operations = fields.Boolean(related='picking_id.user_can_edit_operations', help='True for supervisors/admins: controls location field editability in views.')
    
    user_can_edit_qty = fields.Boolean(related='picking_id.user_can_edit_qty',help='True only for admins: controls quantity field editability in views.')
    ...
    def _is_from_barcode_app(self):
        ...
        try:
            if http_request and http_request.httprequest.path == '/stock_barcode/save_barcode_data':
                _logger.info("[BARCODE DETECT] detected via HTTP path for user=%s ids=%s",self.env.user.login, self.ids)
                return True
    ...
    def write(self, vals):
        ...
        # _logger.info("[ML WRITE] user=%s from_barcode=%s ids=%s vals_keys=%s",self.env.user.login, from_barcode, self.ids, list(vals.keys()))
        ...
        from_pos = self.env.context.get('from_pos_order')

        if writing_qty and not from_barcode and not from_pos and self._user_is_restricted_operator():
            _logger.warning("[ML WRITE] BLOCKED manual qty change: user=%s ids=%s", self.env.user.login, self.ids)
        ...
        # Only operators are blocked from locations, product, lot.
        # Supervisors and admins can freely change location_id / location_dest_id on move lines.
        if self._user_is_restricted_operator():
            locked_structural = {'location_id', 'location_dest_id', 'result_package_id', 'product_id', 'move_id', 'manual_entry_reason',}
